...
- Assume the effect of velocity dispersion is Gaussian. Then, to get a dispersed spectrum, you should convolve the template with a Gaussian kernel with a given radius. Write this in maths, that is $$T(x)\ast G(x, r)$$.
- But we should consider that the featureless continuum components are different between template and galaxy. So we simply add a featureless continuum which is typically a straight line with $$x = \ln(\lambda)$$ (in $$\lambda$$ domain this is an exponential continuum). Hence: $$[T(x)\ast G(x, r)] + C(x)$$.
...
$$M(x) = \{[T(x)\ast G(x, r)] + C(x)\}\sum_l P_l(x).$$
